...
% 1.1   03/26/09    S. Williams require separate passing of timeseries, not
%                               required if not making surrogate data.
%                               make output into structure with info and
%                               output fields, output field will be a
%                               structure array (not cell array of
%                               structures as in power sepctra or
%                               coherencies). do pca on all swatches
%                               concatenated, output is one structure
%                               array, with each field (u,v,s) having a
%                               column for each PCA datatype. concatenation
%                               of spectral & cross-spectral epochs for pca
%                               analysis happens in vertical dimension.
%                               Added 6th PCA datatype (nonfunctional)
%DON'T FORGET TO UPDATE VERSION NUMBER BELOW.

funcname = 'xtp_justPCA.m';
version = 'v1.1';
...
if nargin < 3
    ...
    PCAparams.pca_maxfreq = max(cohgram.output{1}.f);
    PCAparams.pca_ntoplot = 3;
    PCAparams.ifsurr = 0;
end

cohpair = PCAparams.cohpair;
pca_submean = PCAparams.pca_submean;
pca_ntoplot = PCAparams.pca_ntoplot;
pca_maxfreq = PCAparams.pca_maxfreq;
ifsurr = PCAparams.ifsurr;
...
for iswatch = 1:nswatches
    ...
    SG = cat(1, SG, cat(3, cohgram.output{iswatch,cohpair}.S1, cohgram.output{iswatch,cohpair}.S2));   
    S12 = cat(1, S12, cohgram.output{iswatch, cohpair}.S12);        % I could probably do this a little more elegantly.
end
...
%% do PCA on the entire accumulation of epochs at once

iswatch = 1;
f = cohgram.output{iswatch,cohpair}.f;
% what to do with this tvals? how is it used? !!!!!!!!!!!!!!!!!!!!!!!!!
...
pstring_d=sprintf('%s sw%3.0f',fn_aug,iswatch); % will see what this represents!!!!!
...
for ipca=1:pca_datatypes            % store pca values for each type of pca performed
    ...
    switch ipca
        ...
        case 6
            % attempt at normalizing cross spectrum by its mean
            x=max((SG(:,fsample,:)./(ones(size(SG,1),1)*mean(SG(:,fsample,:),2))),min_pwr);
            x=reshape(x,size(x,1),size(x,2)*size(x,3));
            x=[x,max(S12(:,fsample),min_pwr)];
            pca_subsets{1}='normalized S1';
            pca_subsets{2}='normalized S2';
            pca_subsets{3}='S12';
